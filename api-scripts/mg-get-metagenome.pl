...
sub help {
  my $text = qq~
NAME
    mg-get-metagenome.pl -- retrieve a metagenome from the communities API

VERSION
    2

SYNOPSIS
    mg-get-metagenome.pl [ --help, --user <user>, --pass <password>, --token <oAuth token>, --webkey <communities webkey>, --verbosity <verbosity level>--id <metagenome id>]
...
DESCRIPTION
  ...
  Options
    ...
    id - id of the metagenome to be retrieved
...
my $HOST      = 'http://kbase.us/services/communities/1/metagenome/';
my $user      = '';
my $pass      = '';
my $token     = '';
my $verbosity = 'full';

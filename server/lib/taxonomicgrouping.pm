...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "domain", "hidden", "scientific_name", "alias" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$taxonomicgrouping_list, $response->[0]->{$key});
    }
  ...
  if ($rest && scalar(@$rest) == 1) {
    ...
    $taxonomicgrouping->{taxonomicgroupings} = $taxonomicgrouping_ids;
    ...
    $taxonomicgrouping->{genomes} = $genome_ids;

...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "pegs", "rnas", "scientific_name", "complete", "prokaryotic", "dna_size", "contigs", "domain", "genetic_code", "gc_content", "phenotype", "md5", "source_id" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$genome_list, $response->[0]->{$key});
    }
  ...
  if ($rest && scalar(@$rest) == 1) {
    ...
    $genome->{atomicregulons} = $atomicregulon_ids;
    ...
    $response = $response->{result}->[0];
    my $ssrow_ids = [];
    @$ssrow_ids = map { $_->[2]->{id} } @$response;
    $genome->{ssrows} = $ssrow_ids;
    ...
    $response = $json->decode($ua->post($cdmi_url, Content => $content)->content);
    $response = $response->{result}->[0];
    my $taxonomicgrouping_ids = [];
    @$taxonomicgrouping_ids = map { $_->[2]->{id} } @$response;
    $genome->{taxonomicgroupings} = $taxonomicgrouping_ids;

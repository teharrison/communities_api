...
sub about {
  my $content = { 'description' => "A biomass is a collection of compounds in a specific
...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "mod_date", "name" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$biomass_list, $response->[0]->{$key});
    }
    ...
    print $json->encode( $biomass_list );
    exit 0;
  }
  ...
  if (! $rest || ! scalar(@$rest)) {    
    ...
    my $biomass_list = [];
    @$biomass_list = map { keys(%$_) } @$response;
  ...
  if ($rest && scalar(@$rest) == 1) {
    my $data = { 'params' => [ [ $rest->[0] ], [ "id", "mod_date", "name" ] ],
    ...
    my $biomass = $response->{result}->[0]->{$k[0]};
    $biomass->{url} = $cgi->url."/biomass/".$rest->[0];
    ...
    $biomass->{biomasscompounds} = $biomasscompound_ids;
    ...
    $biomass->{models} = $model_ids;
    my $out = $json->encode( $biomass );
    $out =~ s/%7C/|/g;

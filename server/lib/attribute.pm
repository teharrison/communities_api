...
sub about {
  my $content = { 'description' => "An attribute describes a category of condition or characteristic for
...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "description" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$attribute_list, $response->[0]->{$key});
    }
    ...
    print $json->encode( $attribute_list );
    exit 0;
  }
  ...
  if (! $rest || ! scalar(@$rest)) {    
    ...
    my $attribute_list = [];
    @$attribute_list = map { keys(%$_) } @$response;
  ...
  if ($rest && scalar(@$rest) == 1) {
    my $data = { 'params' => [ [ $rest->[0] ], [ "id", "description" ] ],
    ...
    my $attribute = $response->{result}->[0]->{$k[0]};
    $attribute->{url} = $cgi->url."/attribute/".$rest->[0];
    ...
    $attribute->{experiments} = $experiment_ids;
    my $out = $json->encode( $attribute );
    $out =~ s/%7C/|/g;

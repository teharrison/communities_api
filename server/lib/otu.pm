...
sub about {
  my $content = { 'description' => "An OTU (Organism Taxonomic Unit) is a named group of related
genomes.
...
sub request {
  ...
  if ($rest && scalar(@$rest) == 1) {
    ...
    my $otu = $response->{result}->[0]->{$k[0]};
    $otu->{url} = $cgi->url."/otu/".$rest->[0];
    ...
    $otu->{genomes} = $genome_ids;
    my $out = $json->encode( $otu );
    $out =~ s/%7C/|/g;

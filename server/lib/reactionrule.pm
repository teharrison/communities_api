...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "direction", "transproton" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$reactionrule_list, $response->[0]->{$key});
    }
  ...
  if ($rest && scalar(@$rest) == 1) {
    ...
    my $complex_ids = [];
    @$complex_ids = map { $_->[2]->{id} } @$response;
    $reactionrule->{complexs} = $complex_ids;
    ...
    $reactionrule->{compartments} = $compartment_ids;
    ...
    $reactionrule->{reactions} = $reaction_ids;

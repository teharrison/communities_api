...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "stoichiometry", "cofactor", "compartment_index", "transport_coefficient" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$reagent_list, $response->[0]->{$key});
    }
  ...
  if ($rest && scalar(@$rest) == 1) {
    ...
    $reagent->{compounds} = $compound_ids;
    ...
    $reagent->{compartments} = $compartment_ids;
    ...
    $reagent->{reactions} = $reaction_ids;

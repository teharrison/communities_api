...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "compartment_index", "label", "pH", "potential" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$modelcompartment_list, $response->[0]->{$key});
    }
  ...
  if ($rest && scalar(@$rest) == 1) {
    ...
    $modelcompartment->{requirements} = $requirement_ids;
    ...
    $modelcompartment->{biomasscompounds} = $biomasscompound_ids;
    ...
    $response = $response->{result}->[0];
    my $compartment_ids = [];
    @$compartment_ids = map { $_->[2]->{id} } @$response;
    $modelcompartment->{compartments} = $compartment_ids;
    ...
    $modelcompartment->{models} = $model_ids;

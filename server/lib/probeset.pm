...
sub about {
  my $content = { 'description' => "A probe set is a device containing multiple probe sequences for use
...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$probeset_list, $response->[0]->{$key});
    }
    ...
    print $json->encode( $probeset_list );
    exit 0;
  }
  ...
  if (! $rest || ! scalar(@$rest)) {    
    ...
    my $probeset_list = [];
    @$probeset_list = map { keys(%$_) } @$response;
  ...
  if ($rest && scalar(@$rest) == 1) {
    ...
    my $probeset = $response->{result}->[0]->{$k[0]};
    $probeset->{url} = $cgi->url."/probeset/".$rest->[0];
    ...
    $probeset->{atomicregulons} = $atomicregulon_ids;
    ...
    $probeset->{experiments} = $experiment_ids;
    my $out = $json->encode( $probeset );
    $out =~ s/%7C/|/g;

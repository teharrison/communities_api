...
sub about {
  my $content = { 'description' => "A variation describes a set of aligned regions
...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "notes" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$variation_list, $response->[0]->{$key});
    }
    ...
    print $json->encode( $variation_list );
    exit 0;
  }
  ...
  if (! $rest || ! scalar(@$rest)) {    
    ...
    my $variation_list = [];
    @$variation_list = map { keys(%$_) } @$response;
  ...
  if ($rest && scalar(@$rest) == 1) {
    my $data = { 'params' => [ [ $rest->[0] ], [ "id", "notes" ] ],
    ...
    my $variation = $response->{result}->[0]->{$k[0]};
    $variation->{url} = $cgi->url."/variation/".$rest->[0];
    ...
    $variation->{contigs} = $contig_ids;
    my $out = $json->encode( $variation );
    $out =~ s/%7C/|/g;

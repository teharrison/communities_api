...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "alignment_method", "alignment_parameters", "alignment_properties", "tree_method", "tree_parameters", "tree_properties" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$alignmenttree_list, $response->[0]->{$key});
    }
  ...
  if ($rest && scalar(@$rest) == 1) {
    ...
    $alignmenttree->{proteinsequences} = $proteinsequence_ids;

...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$atomicregulon_list, $response->[0]->{$key});
    }
  ...
  if ($rest && scalar(@$rest) == 1) {
    ...
    $atomicregulon->{features} = $feature_ids;
    ...
    $atomicregulon->{genomes} = $genome_ids;
    ...
    $atomicregulon->{probesets} = $probeset_ids;
    ...
    $atomicregulon->{experiments} = $experiment_ids;

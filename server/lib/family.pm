...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "type", "family_function" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$family_list, $response->[0]->{$key});
    }

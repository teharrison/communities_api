...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "coefficient" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$biomasscompound_list, $response->[0]->{$key});
    }
  ...
  if ($rest && scalar(@$rest) == 1) {
    ...
    $biomasscompound->{compounds} = $compound_ids;
    ...
    $biomasscompound->{modelcompartments} = $modelcompartment_ids;
    ...
    $biomasscompound->{biomasss} = $biomass_ids;

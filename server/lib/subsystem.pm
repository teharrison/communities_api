...
sub request {
  ...
  if (exists $cgiparams->{'verbose'}) {
    my $data = { 'params' => [ 0, 1000000, [ "id", "version", "curator", "notes", "description", "usable", "private", "cluster_based", "experimental" ] ],
    ...
    foreach my $key (keys(%{$response->[0]})) {
      push(@$subsystem_list, $response->[0]->{$key});
    }
  ...
  if ($rest && scalar(@$rest) == 1) {
    ...
    $subsystem->{diagrams} = $diagram_ids;
    ...
    $subsystem->{subsystemclasss} = $subsystemclass_ids;
    ...
    $subsystem->{scenarios} = $scenario_ids;
    ...
    $response = $response->{result}->[0];
    my $variant_ids = [];
    @$variant_ids = map { $_->[2]->{id} } @$response;
    $subsystem->{variants} = $variant_ids;

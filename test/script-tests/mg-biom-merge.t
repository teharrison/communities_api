...
sub get_data{
	...
	foreach my $i (@$ids){
		...
		if($create_test_data){
			system("$script --ids $i --source Subsystems --format biom --evalue $value > $test_data_path/$id.$value.mg-compare-functions") unless (-f "$test_data_path/$i.$value.mg-compare-function")
		}

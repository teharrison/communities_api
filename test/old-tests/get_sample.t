...
sub test_sample_query {
	...
	# 2. Let order = "name"
	$cmd = "$test_name -order='name' -limit=10 -offset=0 ";
	$num_tests += &test_response($cmd);	

	# 4. Let order = default
	$cmd = "$test_name  -limit=10 -offset=0 ";
	$num_tests += &test_response($cmd);	

	# 5. Let limit be default
	$cmd = "$test_name  -order='id' -offset=0 ";
	$num_tests += &test_response($cmd);	

	# 6. Let offset be default
	$cmd = "$test_name  -order='id' -limit=10 ";
	$num_tests += &test_response($cmd);	

	# 7. Let offset be nonzero
	$cmd = "$test_name  -order='id' -limit=10 -offset=10 ";
	$num_tests += &test_response($cmd);	
	...
	# --------------------------------------------
	# 2. order = blah
	$cmd = "$test_name  -order='blah' -limit=10 -offset=10 ";
	$num_tests += &bad_response($cmd);	

	# 3. limit = blah
	$cmd = "$test_name  -limit='blah' -offset=10 ";
	$num_tests += &bad_response($cmd);	

	# 4. offset = blah
	$cmd = "$test_name   -limit=10 -offset='blah' ";
	$num_tests += &bad_response($cmd);	
